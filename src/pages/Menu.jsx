    ...
    <div className="min-h-screen">
      ...
      <section className="relative h-96 bg-gradient-to-r from-[#1a1a1a] to-[#2a2a2a] text-white flex items-center">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl text-gray-200">Authentic Mangalorean seafood and South Indian specialties</p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid gap-8">
            {menuData[activeCategory]?.map((item, idx) => (
              <div 
                key={idx}
                className="bg-white rounded-2xl p-8 shadow-lg flex flex-col md:flex-row gap-8 items-center"
              >
                <div className="flex-1">
                  <h3 className="text-2xl font-bold text-[#1a1a1a] mb-2">{item.name}</h3>
                  <p className="text-gray-600 mb-4">{item.description}</p>
                  <p className="text-3xl font-bold text-[#1a1a1a]">{item.price} SAR</p>
                </div>
                <div className="flex-shrink-0">
                  <button className="bg-[#25D366] text-white px-8 py-4 rounded-xl font-semibold hover:bg-[#20bd5a] transition">
                    Add to Order
                  </button>
                </div>
              </div>
